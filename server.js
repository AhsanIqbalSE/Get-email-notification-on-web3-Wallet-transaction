...
app.post("/webhook", async (req,res)=>{
    ...
    try {
        if(body?.txs[0]?.fromAddress){
            ...
            if(body?.txs[0]?.value == '0' && body?.erc20Transfers.length > 0) {
             
                payload ={
                    fromAddress:body?.erc20Transfers[0]?.from,
                    toAddress:body?.erc20Transfers[0]?.to,
                    value: body?.erc20Transfers[0]?.value ? (parseInt(body?.erc20Transfers[0]?.value) / (10 ** 18)):0,
                    symbol: body?.erc20Transfers[0]?.tokenSymbol
                }
                console.log("token");
                console.log(payload);
                
            }else if(body?.txs[0]?.value == '0' && body?.erc20Transfers.length == 0){
                ...
                payload ={
                    fromAddress:response?.result[0]?._data?.fromAddress?._value?.toString(),
                    toAddress:response?.result[0]?._data?.toAddress?._value?.toString(),
